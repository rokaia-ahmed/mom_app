
const BASEURL='https://newMommy.mooo.com:';

const SIGNUP ='3001/api/auth/register' ;
const LOGIN ='3001/api/auth/login' ;
const SENDEMAIL ='3001/api/auth/forgetPassword' ;
const VERIFYEMAIL ='3001/api/auth/verifyResetMessage/' ;
const RESETPASSWORD ='3001/api/auth/resetPassword/' ;
const REFRESHTOKEN ='3001/api/auth/refresh' ;

const ADDBABY ='3002/api/users/addBaby' ;
const ADDACTIVITY ='3002/api/users/addActivity/64ad17abde924f80600fd73f';
const GETALLACTIVITY ='3002/api/users/activities/64ad17abde924f80600fd73f';
const UPDATEACTIVITY='3002/api/users/updateActivity/';
const DELETEACTIVITY='3002/api/users/activity/';

const ADDMEAL ='3002/api/users/addMeal/64ad17abde924f80600fd73f';
const GETALLMEALS ='3002/api/users/meals/64ad17abde924f80600fd73f';
const UPDATEMEAL='3002/api/users/updateMeal/';
const DELETEMEAL='3002/api/users/meal/';

const ADDREMINDER ='3002/api/users/addReminder/64ad22d223d2786cf2852250';
const GETALLREMINDER ='3002/api/users/allReminders/64ad22d223d2786cf2852250';
const UPDATEREMINDER='3002/api/users/updateReminder/';
const DELETEREMINDER='3002/api/users/reminder/';

const ADDBABYFIRST ='3002/api/users/addFirst/64ad211c23d2786cf285224f';
const GETALLBABYFIRST ='3002/api/users/allFirsts/64ad211c23d2786cf285224f';
const UPDATEBABYFIRST='3002/api/users/updateFirst/';
const DELETEBABYFIRST='3002/api/users/first/';

const ADDGROWTH ='3002/api/users/addGrowthMilestone/64d3d3934e9bca8f76dea5bb';
const GETALLGROWTH ='3002/api/users/growthMilestones/64d3d3934e9bca8f76dea5bb';
const UPDATEGROWTH='3002/api/users/updateGrowthMilestone/';
const DELETEGROWTH='3002/api/users/growthMilestone/';

const ADDMEDICALRECORD ='3002/api/users/addMedicalRecord/64d3d3934e9bca8f76dea5bb';
const GETALLMEDICALRECORD ='3002/api/users/medicalHistory/64d3d3934e9bca8f76dea5bb';
const UPDATEMEDICALRECORD='3002/api/users/updateMedicalRecord/';
const DELETEMEDICALRECORD='3002/api/users/medicalRecord/';

const ADDDOCUMENT ='3002/api/users/addMedicalDocument/64d3d3934e9bca8f76dea5bb';
const GETALLDOCUMENT ='3002/api/users/allDocuments/64d3d3934e9bca8f76dea5bb';
const UPDATEDOCUMENT='3002/api/users/updateMedicalDocument/';
const DELETEDOCUMENT='3002/api/users/document/';

const SENDPUBLICPOST='3003/api/postProfile';
const SENDGROUPPOST='3003/api/postGroup/';
const MYGROUPS='3003/api/allMyGroups';
const RECOMMENDEDGROUPS='3003/api/recommendedGroups';
const CREATEGROUP='3003/api/createGroup';
const JOINGROUP='3003/api/joinGroup/';
const LEAVEGROUP='3003/api/leaveGroup/';
const ADDFRIEND='3003/api/sendFriendRequest/';
const UNFRIEND='3003/api/unfriend/';
const AllPosts='3003/api/feed';
const ALLFRIENDS='3003/api/allFriends';
const ALLFRIENDSRequests='3003/api/allSentFriendRequests/';
const LIKE='3003/api/like/';
const UNLIKE='3003/api/unlike/';
const ACCEPTREQUEST='3003/api/acceptFriendRequest/';
const REJECTREQUEST='3003/api/rejectFriendRequest/';
const PROFILEBYID='3003/api/profileById/';
const GROUPBYID='3003/api/groupById/';
const UPDATEPROFILEIMAGE='3003/api/uploadProfileImage';
const UPDATEPROFILECOVER='3003/api/uploadProfileCover';
const UPDATEGROUPIMAGE='3003/api/uploadGroupImage/';
const UPDATEGROUPCOVER='3003/api/uploadGroupCover/';